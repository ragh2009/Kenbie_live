    ...
    public void gettingSocialSignUpDetails() {
        if (isOnline()) {
            ...
            if (locationItem != null) {
                ...
                params.put("country", locationItem.getCountry() + "");
            } else {
                params.put("latitude", "");
...
//        smr.setHeaders(params);
            ...
            if (locationItem != null) {
                ...
                smr.addStringParam("country", locationItem.getCountryName());
                smr.addStringParam("country_name", locationItem.getCountry());
            } else {
                smr.addStringParam("latitude", "");
                smr.addStringParam("longitude", "");
                smr.addStringParam("city", "");
                smr.addStringParam("country", "");
                smr.addStringParam("country_name", "");
            }
            ...
            for (String line : response) {
                try {
                    ...
                    if (jObj.has("status") && jObj.getBoolean("status")) {
                        ...
                        editor.apply();
                        finish();
//                                Glide.with(SignUpActivity.this).load(Constants.BASE_IMAGE_URL + mPref.getString("ProfilePic", "")).apply(RequestOptions.circleCropTransform()).into(profileImg);
                       /* if (jObj.has("success")) {
                            startMainScreen();
                            Toast.makeText(SignUpActivity.this, jObj.getString("success"), Toast.LENGTH_LONG).show();
                        }*/
                    } /*else if (jObj.has("error"))
                        Toast.makeText(getApplicationContext(), jObj.getString("error"), Toast.LENGTH_LONG).show();
                    else
                        Toast.makeText(getApplicationContext(), Constants.GENERAL_FAIL_MSG, Toast.LENGTH_LONG).show();*/
                } catch (Exception e) {
                    e.printStackTrace();
                }
//                showProgressDialog(false);
...
//                                Glide.with(SignUpActivity.this).load(Constants.BASE_IMAGE_URL + mPref.getString("ProfilePic", "")).apply(RequestOptions.circleCropTransform()).into(profileImg);
            ...
            Volley.newRequestQueue(SignUpActivity.this).add(smr);*/
    }

    /* --------------------- OLD Code -----------------------------------*/
   /* // Uploading image on server
    private void uploadImage(final int type) {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, MConnection.API_BASE_URL + "uploadPic",
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
                        try {
                            if (s != null) {
                                ...
                                if (jo.has("user_pic")) {
                                    SharedPreferences.Editor editor = mPref.edit();
                                    editor.putString("ProfilePic", jo.getString("user_pic"));
                                    editor.apply();
                                }
                            }
                        } catch (Exception e) {
            ...
            protected Map<String, String> getParams() {
                ...
                String image = getStringImage(profilePicBitmap);
        ...
        switch (requestCode) {
            case RuntimePermissionUtils.REQUEST_CAMERA:
                if (grantResults != null && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    SignUpStepThree signUpStepThree = (SignUpStepThree)
                            getSupportFragmentManager().findFragmentById(R.id.frame_layout);

                    if (grantResults.length > 1 && grantResults[1] == PackageManager.PERMISSION_GRANTED) {
                        if (signUpStepThree != null)
                            signUpStepThree.startCamera();
                    } else {
                        if (signUpStepThree != null)
                            signUpStepThree.startCamera();
                    }
            ...
            case RuntimePermissionUtils.REQUEST_WRITE_EXTERNAL_STORAGE:
                if (grantResults != null && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    SignUpStepThree signUpStepThree = (SignUpStepThree)
                            getSupportFragmentManager().findFragmentById(R.id.frame_layout);
                    if (signUpStepThree != null)
                        signUpStepThree.openPhoneGallery();
                }
                break;
        }
    }
